...
export class ChatService {
    ...
    async getAllConversationsForUser(userId: number) {
        return await getConnection().query(`
            SELECT distinct m.id,  m.message_text, m.timestamp,  m.conversationId, CASE WHEN c.is_group = 1 THEN c.group_name ELSE CONCAT(u.first_name,' ', u.last_name) END as conversation_name
            FROM message m
            ...
            AND cu.userId != ?
            ORDER BY m.id DESC;`, [userId, userId]);
    }
}
